...
namespace UnityStandardAssets.Vehicles.Car
{
	...
	public class WaypointTracker_pid
	{
		...
		public Vector3 RestartPosition(CarController cc) {

			//first find closest waypoint, then return its position
			int i = NextWaypoint(cc);
			Vector3 position = waypoints[i];
			position.y = 1.726f; //waypoints are too high above the track!
			return position;
		}

		public Quaternion RestartOrientation(CarController cc) {

			//first find closest waypoint, then return its position
			int i = NextWaypoint(cc);
			int j = i-1;
			if (j<0) j = waypoints.Count-1;
			Vector3 direction = waypoints[i] - waypoints[j];
